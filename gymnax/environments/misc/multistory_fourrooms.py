...
numbered_four_rooms_map = jnp.array(
    [
...
def rooms_map_to_multistory(
    map: chex.Array, num_floors: int = 1
) -> Tuple[chex.Array, chex.Array]:
    """Convert numbered room map into multistory layouts (walk and rooms)"""
    walk_map = map.at[map >= 0].set(0)  # Alias the rooms for this layout
    walk_map = walk_map + 1
    ms = jnp.stack([walk_map for _ in range(num_floors)], 0)
    n_rooms = map.max()  # can work with different numbers of rooms
    ms_rooms = jnp.stack(
        [map.at[map >= 0].add(i * n_rooms) for i in range(num_floors)], 0
    )
    if num_floors > 1:
        ms = ms.at[1:, downstairs[0], downstairs[1]].set(2)
        ms = ms.at[:-1, upstairs[0], upstairs[1]].set(3)
    return ms, ms_rooms
...
class MultistoryFourRooms(environment.Environment):
    def __init__(
        ...
        super().__init__()
        self.env_map, self.rooms_map = rooms_map_to_multistory(
            numbered_four_rooms_map, num_floors
        )
        ...
        self.coord_to_state_map = jnp.cumsum(self.occupied_map).reshape(
            self.occupied_map.shape
        )
        self.coords = jnp.stack(jnp.nonzero(self.spawn_map), axis=-1)
        self.directions = jnp.array([[0, -1, 0], [0, 0, 1], [0, 1, 0], [0, 0, -1]])
        self.upstairs = jnp.array(
            [1, *(SW - NE)]
        )  # Move from NE on floor 1 to SW on floor 2
        self.downstairs = jnp.array(
            [-1, *(NE - SW)]
        )  # Move from SW on floor 1 to NE on floor 0
    ...
    def step_env(
        ...
        moved = jnp.any(
            new_pos != p, -1
        )  # Only use stairs if we had to move to this square
    ...
    def reset_env(
        ...
        goal = jax.lax.select(
            params.resample_goal_pos,
            reset_goal(rng_goal, self.available_goals),
            self.goal_fixed,
        )

        pos = jax.lax.select(
            params.resample_init_pos,
            reset_pos(rng_pos, self.available_agent_spawns, goal),
            self.pos_fixed,
        )
...
def reset_goal(rng: chex.PRNGKey, available_goals: chex.Array) -> chex.Array:
