...
@struct.dataclass
class EnvParams:
    listen_success_prob: float = (
        0.85  # probability of listening and hearing tiger at right location
    )
...
class Tiger(environment.Environment):
    ...
    def step_env(
        ...
        d = jax.lax.select(
            s.time >= params.max_steps_in_episode, True, d
        )  # Done if exceeded time limit
        r = jax.lax.select(
            action == state.tiger, params.reward_tiger, params.reward_not_tiger
        )  # Door reward
        r = jax.lax.select(action == 2, params.reward_listen, r)  # listen reward
        return self.get_obs(key, s, action, params), state, r, d, {}

    def reset_env(self, key: chex.PRNGKey, params: EnvParams) -> Tuple[int, EnvState]:
        """Redraw tiger position, first observation is as if we didn't listen"""
        rng_obs, rng_tiger = jax.random.split(key, 2)
        tiger_left = (jax.random.uniform(rng_tiger) > params.tiger_left_prob).astype(
            int
        )
    ...
    def get_obs(
        ...
        o = jax.lax.select(
            jax.random.uniform(key) <= prob_correct_obs, state.tiger, 1 - state.tiger
        )
        return o
